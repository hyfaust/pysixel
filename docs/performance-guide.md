...
早期版本没有输出光栅属性，终端按默认字符单元格比例渲染（约 2:1 高宽比），导致图像垂直拉伸约 2 倍。旧方案是 `char_aspect = 0.5` 预压缩高度，但这是 hack。

### 13.2 正确方案

在 DCS 头后输出光栅属性 `"1;1;W;H`：

```
ESC P 0;0;0q "1;1;W;H #0;2;...
              ^^^^^^^^^
              Pan=1 Pad=1 (1:1 像素宽高比)
              Ph=W  Pv=H  (图像像素尺寸)
```

与 libsixel img2sixel 和 sixel-web 编码器一致。终端收到后自动按 1:1 宽高比渲染，无需任何 hack。

---

## 14. libsixel 移植优化（A/B 类）

基于 libsixel 1.8.7 源码分析，按收益/难度矩阵实施的优化：

### A 类：高收益低难度

| 优化 | 说明 | 效果 |
|------|------|------|
| A1 自动禁用 FS | 15-bit 哈希估算原图独特色数，≤ max_colors 时跳过 FS | 低色图省掉最慢阶段 |
| A2 GIF 调色板缓存 | 第一帧保存调色板，后续帧复用跳过量化 | GIF 动画减少重复计算 |
| A3 RLE 阈值 3 | 与 libsixel 对齐（原为 4） | 输出体积微减 |

### B 类：中收益中难度

| 优化 | 说明 | 效果 |
|------|------|------|
| B1+B2 FS 哈希缓存 | `cachetable[32768]` 15-bit 哈希，O(1) 摊还颜色查找 | FS 颜色匹配从 O(n_colors) 降到 O(1) |
| B3 采样量化 | 像素数 > 1M 时下采样到 ~1000px 做 MEDIANCUT | 大图量化加速 |

### 14.1 FS 哈希缓存实现

```python
cachetable = np.zeros(32768, dtype=np.int16)  # 0=未缓存

# 每个像素:
hash_idx = (R >> 3) << 10 | (G >> 3) << 5 | (B >> 3)
cached = cachetable[hash_idx]
if cached:
    best = cached - 1          # 缓存命中: O(1)
else:
    best = argmin(distances)   # 缓存未命中: O(n_colors)
    cachetable[hash_idx] = best + 1  # +1 偏移，0 表示未缓存
```

与 libsixel 的 `lookup_fast()` 策略一致。自然图像缓存命中率极高（量化后仅 256 色，哈希空间 32768 桶）。


## 15. 关键经验总结（续）

7. **光栅属性是正确做法**：`"1;1;W;H` 让终端按 1:1 像素宽高比渲染，比 `char_aspect=0.5` hack 更正确、更通用。

8. **哈希缓存是 FS 的关键优化**：FS 逐像素循环无法向量化，但颜色查找可以用哈希缓存加速到 O(1)。

9. **采样量化对大图有效**：MEDIANCUT 的质量主要取决于颜色分布而非像素数量，下采样到 ~1000px 足以获得好的调色板。

10. **低色图应跳过 FS**：原图颜色数不超过调色板大小时，量化是无损的，FS 只会添加噪声。
