...
pysixel 最终提供 14 个 CLI 参数，覆盖输出控制、编码质量、性能调优等维度：

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `INPUT` | 输入图片路径（位置参数） | 必填 |
| `-o FILE` | 输出到文件而非 stdout | stdout |
| `-w WIDTH` | 指定输出宽度（像素） | 自适应终端宽度 |
| `-H HEIGHT` | 指定输出高度（像素） | 按比例缩放 |
| `-r METHOD` | 缩放算法：bilinear / lanczos3 | bilinear |
| `-q LEVEL` | 量化质量：low / medium / high | medium |
| `-E MODE` | 编码模式：fast / normal / size | normal |
| `--colors N` | 调色板颜色数（2-256） | 32 |
| `--dither` | 启用 Bayer 有序抖动 | 关闭 |
| `-e` | 单色模式（二值化） | 关闭 |
| `-i` | 底片/反转效果 | 关闭 |
| `-B COLOR` | 透明区域背景填充色 | 无 |
| `-P` | tmux 模式（通过 Sixel pass-through） | 关闭 |
| `-g` | GIF 加速模式（跳过帧间 sleep） | 关闭 |
| `--no-anim` | 强制静态模式，GIF 只显示第一帧 | 关闭 |

### 14.3 参数演进

| 阶段 | 参数数 | 新增参数 |
|------|--------|----------|
| V0 原始版 | 2 | `--no-anim`, `--dither` |
| 最终版 | 14 | `-o`, `-w`, `-H`, `-r`, `-q`, `-E`, `--colors`, `-e`, `-i`, `-B`, `-P`, `-g` |

新增的 12 个参数覆盖了：尺寸控制（`-w`/`-H`/`-r`）、质量调优（`-q`/`--colors`/`--dither`）、编码策略（`-E`）、视觉效果（`-e`/`-i`/`-B`）、输出控制（`-o`/`-P`/`-g`）。
